...
47. **Price Comparison**:
    - Find books priced between $5 and $15.
    - Example SQL: SELECT * FROM BOOK WHERE PRICE BETWEEN 5 AND 15;

48. **Identify Books by ID Range**:
    - Show books with IDs between 1 and 30.
    - Example SQL: SELECT * FROM BOOK WHERE ID BETWEEN 1 AND 30;

49. **Querying Based on User Input**:
    - How do I find books based on user input for title or author?
    - Example SQL: SELECT * FROM BOOK WHERE TITLE LIKE '%{user_input}%' OR AUTHOR LIKE '%{user_input}%';

50. **Counting Titles with Specific Word**:
    - How many titles contain the word 'Guide'?
    - Example SQL: SELECT COUNT(*) FROM BOOK WHERE TITLE LIKE '%Guide%';

You will convert these queries to SQL.
"""
]
...
st.header("Gemini App to Retrieve and Add SQL Data")

# --- Section to create a new table ---
st.subheader("Create a New Table")

# Input fields for table creation
...
create_table_btn = st.button("Create New Table")
...
if submit:
    ...
    response = read_sql_query(sql_query, "book_shop.db")
